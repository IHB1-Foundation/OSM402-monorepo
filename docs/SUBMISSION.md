...
| Trust and safety guardrails | HOLD rules, required checks, intent/cart mandates, escrow constraints | `GitPay - HOLD` comment + contract constraints |
...
2. PASS PR merge -> auto payout (`GitPay - Paid`)
3. HOLD PR merge -> block payout (`GitPay - HOLD`, `/api/payout/execute` returns `409`)
